...
export default function DashboardClient({ params }: { params: { tenantId: string } }) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${
      theme === "DARK" ? "bg-stone-950 text-white" : 
      theme === "VINTAGE" ? "bg-[#f7f9fb] text-black" :
      "bg-background text-on-background"
    } selection:bg-primary/30`}>
      {/* SideNavBar Component */}
      <aside className={`fixed left-0 top-0 h-full w-72 border-r transition-colors duration-500 ${
        ...
        <div className="mt-auto p-8 border-t border-stone-200">
          ...
          >
            <div className={`w-10 h-10 rounded-full overflow-hidden border-2 border-transparent transition-all ${
              theme === "DARK" ? "bg-stone-800 group-hover:border-[#ccff00]" : 
            ...
            <div>
              <p className={`text-xs font-black transition-colors uppercase ${
                theme === "DARK" ? "group-hover:text-[#ccff00]" : 
          ...
          <button
            ...
            className={`mt-6 flex items-center gap-3 transition-colors px-1 ${
              theme === "DARK" ? "text-stone-400 hover:text-red-400" : 
      ...
      {/* TopAppBar Component */}
      <header className={`sticky top-0 z-40 w-full backdrop-blur-xl flex justify-between items-center ml-72 px-12 py-6 max-w-[calc(100%-18rem)] transition-colors duration-500 ${
        theme === "DARK" ? "bg-stone-950/60 border-b border-stone-800" : 
      ...
      {/* Main Content Area */}
      <main className={`ml-72 p-12 min-h-screen transition-colors duration-500 ${
...
function DashboardHome({ theme }: { theme: "LIGHT" | "DARK" | "VINTAGE" }) {
  return (
    <>
      {/* Welcome Hero */}
      <section className={`mb-12 relative overflow-hidden rounded-2xl p-12 flex items-end min-h-[320px] shadow-sm transition-colors duration-500 ${
        theme === "DARK" ? "bg-stone-900" : 
        ...
        <div className="absolute inset-0 z-0">
          ...
          <div className={`absolute inset-0 ${
            theme === "DARK" ? "bg-gradient-to-t from-stone-900 via-stone-900/40 to-transparent" : 
        ...
        <div className="relative z-10 w-full">
          <span className={`font-black tracking-widest text-sm uppercase mb-4 block transition-colors ${
            theme === "DARK" ? "text-[#ccff00]" : 
            theme === "VINTAGE" ? "text-stone-400" :
            "text-[#4f6b28]"
          }`}>Welcome Back, Alex</span>
          <h3 className={`text-7xl font-black tracking-tighter leading-tight max-w-2xl transition-colors ${
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 grid grid-cols-3 gap-6">
          ...
          {/* Next Match Card */}
          <div className={`col-span-3 rounded-2xl p-8 relative overflow-hidden shadow-xl transition-colors duration-500 ${
            theme === "DARK" ? "bg-stone-800 text-white" : 
            theme === "VINTAGE" ? "bg-black text-white" :
            "bg-[#4f6b28] text-white"
          }`}>
            <div className={`absolute right-0 top-0 h-full w-1/3 opacity-10 skew-x-12 translate-x-12 ${
              theme === "DARK" ? "bg-[#ccff00]" : "bg-white"
            }`}></div>
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div>
                <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase mb-4 ${
              ...
              <button className={`px-8 py-4 rounded-full font-black text-xs tracking-widest transition-all uppercase shadow-lg ${
                theme === "DARK" ? "bg-[#ccff00] text-stone-950" : 
                theme === "VINTAGE" ? "bg-white text-black" :
                "bg-white text-[#4f6b28]"
              }`}>
        ...
        <div className="col-span-12 lg:col-span-4">
          <div className={`rounded-2xl p-8 h-full shadow-sm border transition-colors duration-500 ${
            theme === "DARK" ? "bg-stone-950 border-stone-800" : 
            theme === "VINTAGE" ? "bg-white border-transparent" :
            "bg-white border-stone-100"
          }`}>
            <div className="flex justify-between items-center mb-6">
              <h4 className={`font-headline font-black text-xl tracking-tighter uppercase transition-colors ${
                theme === "DARK" ? "text-white" : "text-black"
              }`}>Recent Activity</h4>
              <button className={`font-black text-[10px] tracking-widest uppercase hover:underline ${
                theme === "DARK" ? "text-[#ccff00]" : 
                theme === "VINTAGE" ? "text-black" :
                "text-[#4f6b28]"
              }`}>View All</button>
      ...
      <section className="mt-16">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h4 className={`text-4xl font-black tracking-tighter uppercase transition-colors ${
              theme === "DARK" ? "text-white" : "text-black"
          ...
          <div className="flex gap-2">
            <button className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all ${
              theme === "DARK" ? "border-stone-800 text-[#ccff00] hover:bg-stone-800" : "border-stone-200 text-black hover:bg-stone-50"
            }`}>
      ...
      <section className="mt-16 mb-24">
        <h4 className={`text-4xl font-black tracking-tighter uppercase mb-8 transition-colors ${
          theme === "DARK" ? "text-white" : "text-black"
        }`}>Club News</h4>
        <div className="grid grid-cols-12 gap-8">
          <div className={`col-span-12 md:col-span-7 group cursor-pointer overflow-hidden rounded-2xl relative h-[400px] shadow-lg border transition-colors ${
            theme === "DARK" ? "border-stone-800" : "border-transparent"
          }`}>
            <img
              src="/images/clay_court.png"
              alt="New facilities"
            ...
            <div className="absolute bottom-0 left-0 p-8">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase mb-4 inline-block ${
                theme === "DARK" ? "bg-[#ccff00] text-stone-950" : "bg-white text-black"
              }`}>New Facilities</span>
...
function ThemeSelector({ theme, setTheme }: { theme: "LIGHT" | "DARK" | "VINTAGE", setTheme: (t: "LIGHT" | "DARK" | "VINTAGE") => void }) {
  return (
    <div className={`flex items-center gap-1 p-1 rounded-full border transition-colors duration-500 ${
      theme === "DARK" ? "bg-stone-900 border-stone-800" : 
      theme === "VINTAGE" ? "bg-[#f2f4f6] border-stone-200" :
      "bg-stone-100 border-stone-200"
    }`}>
      <div className={`px-4 py-1.5 flex items-center gap-2 border-r mr-1 transition-colors ${
...
function NavItem({ icon, label, active = false, onClick, theme }: { icon: string; label: string; active?: boolean; onClick?: () => void; theme: "LIGHT" | "DARK" | "VINTAGE" }) {
  return (
    ...
    >
      {active && (
        <div className={`absolute left-0 top-0 bottom-0 w-2 rounded-r-full ${
  ...
  return (
    <div className="flex gap-4 items-center">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
        isDark ? "bg-stone-800 text-[#ccff00]" : 
...
function NewsItem({ title, subtitle, tag, theme }: { title: string; subtitle: string; tag: string; theme: "LIGHT" | "DARK" | "VINTAGE" }) {
  ...
  return (
    <div className="group cursor-pointer flex gap-6 hover:translate-x-1 transition-all duration-300">
      <div className={`w-24 h-24 flex-shrink-0 overflow-hidden rounded-xl relative transition-colors ${
        isDark ? "bg-stone-900" : isVintage ? "bg-[#f7f9fb]" : "bg-stone-100"
      }`}>
      ...
      <div className="py-2">
        <span className={`font-black text-[10px] tracking-widest uppercase transition-colors ${
          isDark ? "text-[#ccff00]" : isVintage ? "text-stone-400" : "text-[#4f6b28]"
        }`}>{tag}</span>
        <h6 className={`text-lg font-black tracking-tight mt-1 group-hover:translate-x-1 transition-all ${
          isDark ? "text-white group-hover:text-[#ccff00]" : "text-black"
        }`}>{title}</h6>
    ...
    <div className="space-y-12">
      ...
      <section>
        <div className="flex justify-between items-center mb-8">
          ...
          <button className={`flex items-center gap-2 px-6 py-2.5 rounded-full font-black text-[10px] uppercase tracking-widest border transition-all ${
            isDark ? "bg-stone-900 border-stone-800 text-[#ccff00]" : 
...
function CourtSection({ title, theme }: { title: string; theme: "LIGHT" | "DARK" | "VINTAGE" }) {
  ...
  return (
    <div className={`rounded-3xl p-8 shadow-sm transition-colors duration-500 ${
      isDark ? "bg-stone-900 border border-stone-800" : 
  ...
  return (
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex justify-between items-center">
        <h2 className={`text-5xl font-black tracking-tighter uppercase transition-colors ${
        ...
        <div className="relative">
          <input
            ...
            className={`border-none rounded-full px-8 py-3 w-80 focus:ring-2 text-sm font-black uppercase tracking-widest transition-colors ${
              isDark ? "bg-stone-900 text-white focus:ring-[#ccff00] placeholder-stone-600" : 
          ...
          />
          <span className={`material-symbols-outlined absolute right-4 top-3 ${
      ...
      {/* Hero Section */}
      <div className="grid grid-cols-12 gap-8">
        <div className={`col-span-12 lg:col-span-8 group relative h-[450px] overflow-hidden rounded-[40px] shadow-2xl border ${
        ...
        <div className={`col-span-12 lg:col-span-4 rounded-[40px] p-10 flex flex-col justify-between shadow-xl transition-colors ${
          isDark ? "bg-stone-900" : 
          isVintage ? "bg-white border border-stone-50" :
          "bg-[#fdfbe6]"
        }`}>
          <div>
            <h4 className={`text-3xl font-black leading-tight mb-4 uppercase transition-colors ${
              isDark ? "text-[#ccff00]" : 
          ...
          <button className={`w-full py-4 border-2 rounded-full text-[10px] font-black tracking-[0.2em] transition-all uppercase ${
            isDark ? "border-[#ccff00] text-[#ccff00] hover:bg-[#ccff00] hover:text-stone-950" : 
      ...
      <section>
        <div className="flex justify-between items-end mb-12">
          <h3 className={`text-5xl font-black tracking-tighter uppercase transition-colors ${
            isDark ? "text-white" : "text-black"
          }`}>
            TRAINING TRACKS
          </h3>
          <div className="flex gap-8 text-[10px] font-black tracking-widest uppercase text-stone-400">
            <span>FILTER BY:</span>
            <button className={`pb-1 transition-colors ${
              isDark ? "text-[#ccff00] border-b-2 border-[#ccff00]" : 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Active Clinics */}
          <div className={`rounded-[40px] overflow-hidden flex flex-col group shadow-lg transition-colors border ${
            ...
            <div className="p-10 flex-1 relative">
              <div className={`absolute right-10 top-10 w-12 h-12 rounded-full flex items-center justify-center shadow-md transition-colors ${
                isDark ? "bg-stone-800 text-[#ccff00]" : "bg-white text-black"
              }`}>
                <span className="material-symbols-outlined">bolt</span>
              </div>
              <h4 className={`text-3xl font-black mb-4 uppercase transition-colors ${
                isDark ? "text-white" : isVintage ? "text-black" : "text-[#4f6b28]"
              }`}>ACTIVE CLINICS</h4>
              <p className={`text-sm font-medium leading-relaxed mb-8 transition-colors ${
                isDark ? "text-stone-400" : isVintage ? "text-stone-500" : "text-[#4f6b28]/60"
              }`}>
              ...
              <div className="flex justify-between items-center mt-auto">
                ...
                <button className={`w-14 h-14 rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg ${
                  isDark ? "bg-[#ccff00] text-stone-950" : isVintage ? "bg-black text-white" : "bg-[#4f6b28] text-white"
                }`}>
          ...
          {/* Junior Academy */}
          <div className={`rounded-[40px] overflow-hidden flex flex-col group shadow-lg transition-colors ${
            isDark ? "bg-stone-900 border border-stone-800" : 
            isVintage ? "bg-white border border-stone-50" :
            "bg-[#cfff00]"
          }`}>
            <div className="h-64 overflow-hidden relative">
              ...
              <div className={`absolute top-6 left-6 px-4 py-1 backdrop-blur rounded-full text-[8px] font-black tracking-widest uppercase ${
                isDark ? "bg-stone-900/90 text-[#ccff00]" : "bg-white/90 text-black"
              }`}>
                PREMIER LEVEL
              </div>
            </div>
            <div className="p-10 flex-1">
              <h4 className={`text-3xl font-black mb-4 uppercase leading-none transition-colors ${
                isDark ? "text-white" : "text-[#1a1a1a]"
              }`}>JUNIOR<br />ACADEMY</h4>
              <p className={`text-sm font-medium leading-relaxed mb-8 transition-colors ${
                isDark ? "text-stone-400" : "text-black/60"
              }`}>
                Developing the next generation of competitors. Age groups 8-16.
              </p>
              <button className={`w-full py-4 rounded-2xl text-[10px] font-black tracking-widest transition-all uppercase ${
                isDark ? "bg-[#ccff00] text-stone-950" : "bg-black text-white"
              }`}>
                EXPLORE PATHWAY
              </button>
            </div>
          ...
          <div className={`rounded-[40px] overflow-hidden flex flex-col group shadow-lg transition-colors border ${
            ...
            <div className="p-10 pb-0">
              <div className="flex justify-between items-start mb-4">
                <h4 className={`text-3xl font-black uppercase leading-none transition-colors ${
                  isDark ? "text-white" : isVintage ? "text-black" : "text-[#4f6b28]"
                }`}>SOCIAL<br />MIXERS</h4>
              ...
              <p className={`text-sm font-medium leading-relaxed mb-6 transition-colors ${
                isDark ? "text-stone-400" : isVintage ? "text-stone-500" : "text-[#4f6b28]/60"
              }`}>
              ...
              <div className="flex items-center -space-x-3 mb-8">
                ...
                <div className={`h-8 px-2 text-white text-[8px] font-black flex items-center justify-center rounded-full border-2 transition-colors ${
                  isDark ? "bg-stone-800 border-stone-900" : "bg-black border-white"
                }`}>
                  +14
                </div>
              </div>
            ...
            <div className="h-48 overflow-hidden relative mt-auto">
              ...
              <div className={`absolute right-6 bottom-6 w-12 h-12 rounded-2xl flex items-center justify-center shadow-xl transition-colors ${
                isDark ? "bg-[#ccff00] text-stone-950" : "bg-black text-white"
              }`}>
      ...
      <section className={`rounded-[40px] p-16 grid grid-cols-12 gap-12 transition-colors border ${
        ...
        <div className="col-span-12 lg:col-span-5 space-y-8">
          <div className="space-y-4">
            <h3 className={`text-6xl font-black tracking-tighter uppercase leading-none transition-colors ${
              isDark ? "text-white" : "text-black"
            }`}>
              SPRING<br />SESSION '24
            </h3>
            <p className={`font-medium leading-relaxed max-w-sm transition-colors ${
        ...
        <div className="col-span-12 lg:col-span-7 grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
            ...
            <div key={i} className={`p-8 rounded-3xl group cursor-pointer hover:shadow-xl transition-all border ${
              ...
              <h5 className={`text-lg font-black tracking-tighter mb-6 group-hover:translate-x-1 transition-all uppercase leading-tight ${
                isDark ? "text-white group-hover:text-[#ccff00]" : "text-stone-900 group-hover:text-black"
              }`}>{item.title}</h5>
              <div className="flex justify-between items-center">
                <span className={`px-3 py-1 rounded-full text-[8px] font-black tracking-widest uppercase ${item.badgeColor}`}>
                  {item.badge}
                </span>
                <span className={`material-symbols-outlined text-stone-300 group-hover:translate-x-1 transition-all ${
                  isDark ? "group-hover:text-[#ccff00]" : "group-hover:text-black"
                }`}>chevron_right</span>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="max-w-4xl space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center gap-10">
        <div className={`w-40 h-40 rounded-[40px] overflow-hidden border-4 shadow-2xl transition-colors ${
          isDark ? "border-stone-800" : "border-white"
        }`}>
          <img
            src="https://lh3.googleusercontent.com/aida-public/AB6AXuCIVNG3lcWVm-Ge5NEEZUf-GdmgLwhFzcFnGsboAMqruvOsGoG2KsUaJnNi7egzkBHc-8ccIDPAhhUoKLhZ-6htVuQieJX6w20tMHdUP6wvr91JZaIcvqIJEmHuGFa4z4EtafMvMDZVDCE0FvjKCsjs2BQO27LBpb-zAw7Vj2lY1t1lbEH1wcnRQt6l-9LceLngmvluUeTcJdDm9RVYiiwiCLuDdYSnjSgJK13-P326RgshwnopS9Qa-T0LE8kRyriIPjwU5NIlUVY"
            alt="Profile"
        ...
        <div>
          <div className={`text-[10px] font-black tracking-[0.3em] uppercase mb-2 transition-colors ${
            isDark ? "text-stone-500" : isVintage ? "text-stone-400" : "text-[#4f6b28]"
          }`}>Member Profile</div>
          <h2 className={`text-6xl font-black tracking-tighter uppercase leading-none transition-colors ${
            isDark ? "text-white" : "text-black"
          }`} style={{ fontFamily: 'Lexend, sans-serif' }}>
            ALEX STERLING
          </h2>
          <div className="flex gap-4 mt-6">
            <span className={`px-5 py-2 rounded-full text-[10px] font-black tracking-widest uppercase transition-colors ${
              isDark ? "bg-stone-800 text-[#ccff00]" : isVintage ? "bg-black text-white" : "bg-[#4f6b28] text-white"
            }`}>
              GOLD TIER MEMBER
            </span>
            <span className={`px-5 py-2 border-2 rounded-full text-[10px] font-black tracking-widest uppercase transition-colors ${
              isDark ? "border-stone-800 text-stone-500" : isVintage ? "border-black text-black" : "border-[#4f6b28] text-[#4f6b28]"
            }`}>
              NTRP 4.5
            </span>
          </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        <div className={`col-span-12 lg:col-span-7 rounded-[40px] p-12 shadow-xl space-y-10 transition-colors border ${
          isDark ? "bg-stone-900 border-stone-800" : 
          isVintage ? "bg-white border-stone-50" :
          "bg-[#fdfbe6] border-transparent"
        }`}>
          <h3 className={`text-3xl font-black tracking-tighter uppercase transition-colors ${
            isDark ? "text-[#ccff00]" : "text-black"
          }`}>Account Security</h3>
          <div className="grid grid-cols-2 gap-10">
            {[
              ...
              <div key={i}>
                <div className={`text-[10px] font-black mb-1 tracking-widest transition-colors ${
                  isDark ? "text-stone-600" : "text-stone-400"
                }`}>{item.label}</div>
                <div className={`font-black text-lg transition-colors ${
                  isDark ? "text-white" : "text-black"
                }`}>{item.value}</div>
              </div>
            ))}
          ...
          <div className="pt-8 flex gap-4">
            <button className={`flex-1 py-4 rounded-2xl text-[10px] font-black tracking-widest uppercase transition-colors ${
              isDark ? "bg-stone-800 text-[#ccff00]" : isVintage ? "bg-black text-white" : "bg-[#4f6b28] text-white"
            }`}>
              EDIT INFORMATION
            </button>
            <button className={`flex-1 py-4 border-2 rounded-2xl text-[10px] font-black tracking-widest uppercase transition-colors ${
              isDark ? "border-stone-800 text-white hover:bg-stone-800" : isVintage ? "border-black text-black hover:bg-black hover:text-white" : "border-[#4f6b28] text-[#4f6b28] hover:bg-[#4f6b28] hover:text-white"
            }`}>
              CHANGE PASSWORD
            </button>
          </div>
        ...
        <div className={`col-span-12 lg:col-span-5 rounded-[40px] p-12 shadow-xl flex flex-col justify-between transition-colors border ${
          isDark ? "bg-stone-950 border-stone-800" : 
          isVintage ? "bg-white border-stone-50" :
          "bg-white border-stone-100"
        }`}>
          <div>
            <h3 className={`text-3xl font-black tracking-tighter uppercase mb-8 transition-colors ${
              isDark ? "text-white" : "text-black"
            }`}>Performance</h3>
            <div className="space-y-8">
              ...
              <div className={`h-3 rounded-full overflow-hidden transition-colors ${
                ...
                <div className={`h-full w-[74%] rounded-full shadow-lg transition-colors ${
                  isDark ? "bg-[#ccff00]" : isVintage ? "bg-black" : "bg-[#cfff00]"
                }`}></div>
              </div>
          ...
          <button className={`mt-12 w-full py-4 rounded-2xl text-[10px] font-black tracking-widest transition-colors uppercase ${
            isDark ? "bg-[#ccff00] text-stone-950" : "bg-black text-white"
          }`}>
            VIEW ANALYTICS
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <div key={i} className={`${plan.color} rounded-[40px] p-12 shadow-2xl relative flex flex-col transition-all hover:scale-105`}>
            {plan.popular && (
              <div className={`absolute -top-4 left-12 px-6 py-2 text-[10px] font-black tracking-[0.2em] rounded-full shadow-lg transition-colors ${
                isDark ? "bg-white text-black" : "bg-[#cfff00] text-black"
              }`}>
                MOST POPULAR
              </div>
            )}
            ...
            <button className={`mt-12 w-full py-5 rounded-2xl text-[10px] font-black tracking-widest transition-all uppercase ${
              plan.popular ? (isDark ? "bg-stone-900 text-white" : isVintage ? "bg-white text-black" : "bg-white text-[#4f6b28]") : 
    ...
    <div className="max-w-2xl space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-8">
        {[
          ...
          <div key={i} className={`flex items-center justify-between p-8 rounded-3xl border shadow-sm transition-all border ${
            isDark ? "bg-stone-900 border-stone-800" : 
            isVintage ? "bg-white border-stone-50" :
            "bg-white border-stone-100"
          }`}>
            <div className="flex items-center gap-6">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${
                isDark ? "bg-stone-800 text-[#ccff00]" : "bg-stone-50 text-[#4f6b28]"
              }`}>
                <span className="material-symbols-outlined">{opt.icon}</span>
              </div>
              <div>
                <h4 className={`font-black text-lg uppercase transition-colors ${
                  isDark ? "text-white" : "text-stone-900"
                }`}>{opt.title}</h4>
                <p className="text-stone-400 text-sm font-medium">{opt.desc}</p>
              </div>
            </div>
            <button className={`w-14 h-8 rounded-full relative transition-colors ${
              opt.active ? (isDark ? "bg-[#ccff00]" : isVintage ? "bg-black" : "bg-[#4f6b28]") : "bg-stone-200"
            }`}>
