...
export default function MemberAdminView({ theme = "LIGHT", tenantId }: { theme?: "LIGHT" | "DARK" | "VINTAGE", tenantId?: string | null }) {
  ...
  const inputCls = `w-full border rounded-2xl px-5 py-3.5 text-sm font-bold outline-none transition-all ${
    theme === "DARK" ? "bg-stone-50 text-white border-stone-200 focus:border-[#ccff00]" : "bg-white text-stone-900 border-stone-200 focus:border-stone-400 shadow-sm"
  }`;
  const readonlyCls = `w-full border rounded-2xl px-5 py-3.5 text-sm font-bold transition-colors ${
    theme === "DARK" ? "bg-stone-100 text-stone-200 border-stone-200" : "bg-stone-50 text-stone-800 border-stone-100"
  }`;
        ...
        return (
          <div className="flex justify-center">
            <div className={`w-10 h-10 rounded-full overflow-hidden border-2 transition-colors ${
              theme === "DARK" ? "border-stone-200" : "border-stone-100"
            }`}>
              {url ? (
                <img src={url} alt="User" className="w-full h-full object-cover" />
              ) : (
                <div className={`w-full h-full flex items-center justify-center ${
                  theme === "DARK" ? "bg-stone-100 text-stone-700" : "bg-stone-50 text-stone-300"
                }`}>
            ...
            {allRoleIds.map(rid => {
              ...
              if (!roleMatch) return (
                <span key={rid} className={`text-[9px] font-mono px-2 py-0.5 rounded-full border border-stone-200 transition-colors ${
                  theme === "DARK" ? "border-stone-200 text-stone-200" : "text-stone-950"
                }`}>{rid}</span>
              );
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div>
          ...
          <p className={`font-bold uppercase tracking-widest text-xs mt-2 transition-colors duration-500 ${
            ...
            Platform Management · <span className={
              ...
              theme === "VINTAGE" ? "text-black" :
              "text-[#4f6b28]"
            }>{filteredUsers.length}</span> Members Active
          </p>
        </div>
          ...
          <div className="relative w-80">
            ...
            <input 
              ...
              className={`w-full border rounded-full pl-12 pr-6 py-3 text-sm font-medium outline-none transition-all shadow-sm ${
                theme === "DARK" 
                  ? "bg-stone-100 border-stone-200 text-white focus:border-[#ccff00] placeholder:text-stone-500" 
                  : theme === "VINTAGE"
                  ...
                  {row.getVisibleCells().map(cell => (
                    <td 
                      ...
                      className={`px-6 py-3 text-sm font-medium border-r last:border-r-0 transition-colors duration-500 ${
                        theme === "DARK" ? "text-stone-200 border-stone-200" : 
                        theme === "VINTAGE" ? "text-black border-stone-100" :
          ...
          <div className="flex gap-4">
            <button 
              ...
              className={`flex-1 py-4 border-2 rounded-2xl text-[10px] font-black tracking-widest transition-all uppercase ${
                theme === "DARK" ? "border-stone-200 text-stone-200 hover:bg-stone-100" : 
        ...
        footer={
          <button 
            ...
            className={`w-full py-4 rounded-2xl text-[10px] font-black tracking-widest transition-all uppercase shadow-lg ${
              theme === "DARK" ? "bg-[#ccff00] text-black" : "bg-black text-white"
            }`}
          >
            Done
          </button>
      ...
      >
        <div className="space-y-8 text-center py-4">
          ...
          <div className={`p-6 rounded-2xl border text-left space-y-4 ${
            theme === "DARK" ? "bg-stone-100 border-stone-200" : "bg-stone-50 border-stone-100"
          }`}>
            ...
            <div className={`p-4 rounded-xl font-mono text-[10px] break-all border transition-colors ${
              theme === "DARK" ? "bg-stone-50 border-stone-200 text-[#ccff00]" : "bg-white border-stone-200 text-blue-600"
            }`}>
              {invitationLink}
            </div>
            <button 
      ...
      >
        {(() => {
          ...
          const inputCls = `w-full border rounded-2xl px-5 py-3.5 text-sm font-bold outline-none transition-colors ${
            theme === "DARK" ? "bg-stone-50 text-white border-stone-200 focus:border-[#ccff00]" : "bg-white text-stone-900 border-stone-200 focus:border-stone-400"
          }`;
          const readonlyCls = `w-full border rounded-2xl px-5 py-3.5 text-sm font-bold transition-colors ${
            theme === "DARK" ? "bg-stone-100 text-stone-200 border-stone-200" : "bg-stone-50 text-stone-800 border-stone-100"
          }`;
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="relative group">
                  <div className={`w-32 h-32 rounded-full overflow-hidden flex items-center justify-center border-4 shadow-2xl transition-all relative ${
                    theme === "DARK" ? "border-stone-200 bg-stone-100" : "border-stone-100 bg-stone-50"
                  }`}>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all text-white z-10 gap-4">
                      <button
                        onClick={() => {
                          const fileInput = document.getElementById('member-portrait-upload') as HTMLInputElement;
                          fileInput?.click();
                        }}
                  ...
                  {formData.portrait_url && (
                    <button
                      onClick={() => {
                        ...
                        if (editingUser) handlePortraitSync("");
                      }}
                  ...
                  <button
                    ...
                    className={`text-[9px] font-black tracking-[0.2em] uppercase transition-all flex items-center gap-2 px-4 py-2 rounded-full border ${
                      theme === "DARK" 
                        ...
                        : "border-[#4f6b28]/20 text-[#4f6b28] hover:bg-[#4f6b28]/10"
                    }`}
                  >
                        ...
                        <button
                          ...
                          className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 transition-all flex items-center gap-2 ${
                            active
                              ...
                              : (theme === "DARK" ? "border-stone-200 text-stone-200 hover:border-stone-600" : "border-stone-200 text-stone-950 hover:border-stone-400")
                          }`}
                        >
                    ...
                    <div className="col-span-2">
                      ...
                      <div className={`rounded-2xl border overflow-hidden mt-1 ${theme === "DARK" ? "border-stone-200" : "border-stone-200"}`}>
                        ...
                        {SLOTS.map((slot, si) => (
                          ...
                          >
                            <div className={`px-3 py-3 text-[9px] font-black uppercase tracking-widest flex items-center ${theme === "DARK" ? "text-stone-200 bg-stone-100/50" : "text-stone-950 bg-stone-50/50"}`}>
                              {slot}
                            </div>
                            {DAYS.map(day => {
      ...
      >
        <div className="space-y-6">
          <p className={`text-sm leading-relaxed ${theme === "DARK" ? "text-stone-200" : "text-stone-950"}`}>
            This will create a member in your company responsible for monitoring operations and managing users associated with your business processes.
          </p>
          ...
          {(() => {
            ...
            return (
              <div className="space-y-6">                {/* Portrait — centered above grid */}
                <div className="flex flex-col items-center gap-4">
                  <div className="relative group">
                    <div className={`w-32 h-32 rounded-full overflow-hidden flex items-center justify-center border-4 shadow-2xl transition-all relative ${
                      theme === "DARK" ? "border-stone-200 bg-stone-100" : "border-stone-100 bg-stone-50"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all text-white z-10 gap-4">
                        <button
                          onClick={() => {
                            const fileInput = document.getElementById('create-member-portrait-upload') as HTMLInputElement;
                            fileInput?.click();
                          }}
                          disabled={isUploadingPortrait}
                          className="flex flex-col items-center hover:scale-110 transition-transform bg-black/40 w-12 h-12 rounded-full justify-center backdrop-blur-sm"
                    ...
                    <input
                      id="create-member-portrait-upload"
                      type="file"
                      accept="image/*"
                      className="hidden"
                      disabled={isUploadingPortrait}
      ...
      >
        <div className="space-y-10 py-4">
          <div className="grid grid-cols-4 gap-6">
            {defaultPortraits.map((portrait) => (
              <button
                ...
                className={`group relative aspect-square rounded-[32px] overflow-hidden border-4 transition-all hover:scale-105 active:scale-95 ${
                  theme === "DARK" 
                    ? "border-stone-100 bg-stone-100 hover:border-[#ccff00]" 
                    : "border-white bg-white hover:border-[#4f6b28] shadow-md hover:shadow-xl"
                } ${formData.portrait_url === portrait.url ? (theme === "DARK" ? "border-[#ccff00]" : "border-[#4f6b28]") : ""}`}
              >
                <img 
                  src={portrait.url} 
                  alt={portrait.label} 
                ...
                {formData.portrait_url === portrait.url && (
                  <div className={`absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center shadow-lg animate-in zoom-in-50 duration-300 ${
                    theme === "DARK" ? "bg-[#ccff00] text-black" : "bg-[#4f6b28] text-white"
                  }`}>
            ...
            {defaultPortraits.length === 0 && (
              <div className={`col-span-4 py-20 text-center rounded-[40px] border-2 border-dashed ${
                theme === "DARK" ? "border-stone-200 text-stone-500" : "border-stone-100 text-stone-400"
              }`}>
          ...
          <div className={`p-8 rounded-[40px] flex items-center justify-between gap-6 transition-colors ${
            theme === "DARK" ? "bg-stone-100/50" : "bg-stone-50"
          }`}>
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                theme === "DARK" ? "bg-stone-800 text-[#ccff00]" : "bg-white text-[#4f6b28] shadow-sm"
              }`}>
            ...
            <button
              ...
              className={`px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${
                theme === "DARK" ? "bg-stone-800 text-white hover:bg-stone-700" : "bg-white text-stone-900 border hover:bg-stone-50 shadow-sm"
              }`}
            >
              Close
            </button>
