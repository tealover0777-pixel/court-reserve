...
export default function EventsAdminView({ theme = "LIGHT", tenantId }: { theme?: "LIGHT" | "DARK" | "VINTAGE", tenantId: string }) {
  ...
  useEffect(() => {
    ...
    const usersQ = query(collection(db, "tenants", tenantId, "users"), orderBy("first_name", "asc"));
  ...
  const handleSaveEvent = async (force = false) => {
    ...
    if (!force) {
      ...
      const eventEnd = formData.end_date ? new Date(formData.end_date) : new Date(eventStart.getTime() + 2 * 60 * 60 * 1000);
      
      const q = query(
        collection(db, "tenants", tenantId, "bookings"),
        where("date", "==", eventStart.toDateString())
      );
      
      const snap = await getDocs(q);
      const overlapping = snap.docs.map(d => ({ id: d.id, ...d.data() })).filter((b: any) => {
        const bStart = timeToMinutes(b.time);
        const bEnd = bStart + (b.duration * 60);
        const eStart = eventStart.getHours() * 60 + eventStart.getMinutes();
        const eEnd = eventEnd.getHours() * 60 + eventEnd.getMinutes();
        ...
        return (
          <div className=\"flex flex-col gap-1\">
            <div className=\"flex justify-between items-center text-[10px] font-black\">
              <span>{count}/{max}</span>
              <span className={count >= max ? \"text-red-500\" : \"text-green-500\"}>
                {count >= max ? \"FULL\" : \"OPEN\"}
              </span>
            </div>
            <div className=\"w-24 h-1.5 bg-stone-100 rounded-full overflow-hidden\">
              <div 
                className={`h-full transition-all ${count >= max ? \"bg-red-500\" : \"bg-[#ccff00]\"}`}
        ...
        <div className=\"flex items-center gap-2\">
          <button
            onClick={() => {
              ...
              const ed = event.end_date?.toDate ? event.end_date.toDate() : (event.end_date ? new Date(event.end_date) : null);
              setFormData({
                title: event.title,
                description: event.description,
                date: format(d, "yyyy-MM-dd'T'HH:mm"),
                end_date: ed ? format(ed, "yyyy-MM-dd'T'HH:mm") : "",
                type: event.type,
                max_participants: event.max_participants,
                cancellation_policy: event.cancellation_policy,
                cancellation_deadline: event.cancellation_deadline?.toDate ? format(event.cancellation_deadline.toDate(), "yyyy-MM-dd'T'HH:mm") : (event.cancellation_deadline ? format(new Date(event.cancellation_deadline), "yyyy-MM-dd'T'HH:mm") : ""),
                image_url: event.image_url || "",
                tag: event.tag || "Social",
                event_leaders: event.event_leaders || []
              });
              setShowEditModal(true);
            }}
            className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
              theme === \"DARK\" ? \"hover:bg-stone-800 text-white\" : \"hover:bg-stone-100 text-stone-900\"
            }`}
          >
  ...
  return (
    <div className=\"space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700\">
      <div className=\"flex justify-between items-center\">
        <div>
          <h2 className={`text-4xl font-black tracking-tighter uppercase ${theme === \"DARK\" ? \"text-white\" : \"text-black\"}`}>
            EVENT ADMINISTRATION
          </h2>
          <p className={`text-xs font-black uppercase tracking-widest mt-1 ${theme === \"DARK\" ? \"text-[#ccff00]\" : \"text-stone-500\"}`}>
            Manage Club Events & Sign-ups
          </p>
        </div>
        <button
          onClick={() => {
          ...
          className={`px-8 py-4 rounded-2xl font-black text-xs tracking-widest uppercase transition-all shadow-xl flex items-center gap-3 ${
            theme === \"DARK\" ? \"bg-[#ccff00] text-stone-950 shadow-[#ccff00]/10\" : \"bg-stone-900 text-white shadow-stone-900/10\"
          }`}
        >
      ...
      <div className={`rounded-[32px] border overflow-hidden transition-colors ${
        theme === \"DARK\" ? \"bg-stone-900 border-stone-800\" : \"bg-white border-stone-100 shadow-sm\"
      }`}>
