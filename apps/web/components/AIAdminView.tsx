...
export default function AIAdminView({ theme = "LIGHT" }: { theme?: "LIGHT" | "DARK" }) {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex justify-between items-end mb-10">
        <div>
          ...
          <p className={`font-bold uppercase tracking-widest text-xs mt-2 transition-colors duration-500 ${theme === "DARK" ? "text-stone-400" : "text-stone-900"}`}>
            Intelligence Engine · <span className={theme === "DARK" ? "text-[#ccff00]" : "text-[#4f6b28]"}>Active</span> Status · <span className={theme === "DARK" ? "text-[#ccff00]" : "text-[#4f6b28]"}>v2.4.1</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className={`col-span-2 border rounded-3xl p-12 shadow-sm transition-colors duration-500 ${
          ...
          <div className="flex items-center gap-6 mb-10">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-colors ${
              theme === "DARK" ? "bg-stone-900 text-[#ccff00]" : "bg-stone-50 text-[#4f6b28]"
            }`}>
          ...
          <div className="space-y-8">
            <div className={`flex items-center justify-between p-6 rounded-2xl border transition-colors ${
              theme === "DARK" ? "bg-stone-900/40 border-stone-800" : "bg-stone-50 border-stone-100"
            }`}>
              <div>
              ...
              <div className="w-48 h-2 bg-stone-200 rounded-full relative">
                <div className={`absolute left-0 top-0 h-full w-[70%] rounded-full ${
                  theme === "DARK" ? "bg-[#ccff00]" : "bg-[#4f6b28]"
                }`}></div>
        ...
        <div className={`rounded-3xl p-12 text-white shadow-xl flex flex-col justify-between transition-colors duration-500 ${
          theme === "DARK" ? "bg-stone-900 shadow-black/20" : "bg-[#4f6b28] shadow-[#4f6b28]/20"
        }`}>
          <div>
            <span className={`material-symbols-outlined text-5xl mb-6 transition-colors ${
              theme === "DARK" ? "text-[#ccff00]" : "text-white"
            }`}>bolt</span>
          ...
          <button className={`w-full py-4 rounded-2xl text-[10px] font-black tracking-widest transition-all uppercase mt-12 ${
            theme === "DARK" ? "bg-[#ccff00] text-stone-950 hover:opacity-90" : "bg-white text-[#4f6b28] hover:bg-stone-50"
          }`}>
            Run Diagnostics
          </button>
        </div>
