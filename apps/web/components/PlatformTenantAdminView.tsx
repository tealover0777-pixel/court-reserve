...
export default function PlatformTenantAdminView({ theme = "LIGHT" }: { theme?: "LIGHT" | "DARK" }) {
  ...
  const columns = [
    ...
    columnHelper.accessor("created_at", {
      header: "CREATED AT",
      size: 150,
      cell: info => <span className={`text-xs uppercase font-bold transition-colors duration-500 ${theme === "DARK" ? "text-stone-500" : "text-stone-500"}`}>{info.getValue()}</span>,
    }),
      ...
      <div className="flex justify-between items-end mb-10">
        <div>
          ...
          <p className={`font-bold uppercase tracking-widest text-xs mt-2 transition-colors duration-500 ${theme === "DARK" ? "text-stone-400" : "text-stone-900"}`}>
            Platform Infrastructure · <span className={theme === "DARK" ? "text-[#ccff00]" : "text-[#4f6b28]"}>{tenants.length}</span> Registered Tenants
          </p>
        </div>
